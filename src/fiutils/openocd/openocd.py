...
class OpenOcd:
    ...
    def write_memory(self, address, data, word_length=32):
        ...
        # self.send('array2mem %s 0x%x 0x%x %d' % (self._tcl_variable,
        #     word_length, address, len(data)))

        array = ' '.join(f'0x{x:x}' for x in data)
        
        cmd = 'write_memory 0x%x %d { %s }' % (address, word_length, array)
        print(cmd)
        self.send(cmd)
    ...
    def read_memory(self, address, count, word_length=32):
        ...
        cmd = 'read_memory 0x%x %d %d' % (address, word_length, count)
        # print(cmd)
        raw = self.send(cmd)

        addrs = [f'0x{x:x}' for x in range(address, address + count * (word_length//8), word_length//8)]
        assert len(addrs) == len(raw.split(' '))

        return list(zip(addrs, raw.split(' ')))
