...
def pio_match_one_data():
    ...
    jmp(x_not_y, "start")
    set(pins, 1)
    irq(1) # irq for other sm
    irq(0).delay(0b111) # add some delay to make trigger show up in logic analyzer
    set(pins, 0)
    label("done")
...
sm2 = StateMachine(1, pio_high_counter, 
                   sideset_base=Pin(CROWBAR_OUT, Pin.OUT))
def run2(wait_time_s=1e-3, high_time_s=5e-3):
    sm2.restart()
    sm2.put(int(wait_time_s/(1/freq())))
    sm2.put(int(high_time_s/(1/freq())))
    sm2.active(1)

def run(needle=0, wait_time_s=1e-3, high_time_s=5e-3):
    run2(wait_time_s, high_time_s)
    run1(needle)
